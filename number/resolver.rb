module Number::Resolver # < Array
  ...
  def gout
    pp(map { |grp| grp.ability.map { |abl| [grp.g, abl] if abl[0] > 0 }.compact })
  end
  ...
  def reserv(v_num)
    ...
    @groups.each do |group|
      group.ability.combination_of_ability_of_rest_is_less_or_equal(v_num) # [[[2,[28,29],7], [2,[28,29],9]]]
        ...
        rm_value = @val - values
        next unless (rm_cells.map { |c| @cells[c].ability }.flatten - values).size > 0
  ...
  def groups_remain_2_or_m_cells_of_value_is(h_v, v_h, v)
    @groups.map do |grp|
      next unless grp.type == h_v and # 　　 :holizontal なgroupについて、
